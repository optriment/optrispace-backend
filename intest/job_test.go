...
func TestJob(t *testing.T) {
	...
	t.Run("post•duration is negative", func(t *testing.T) {
		body := `{
			"title":"Create awesome site",
			"description": "There are words here. Very many words.",
			"budget": 100.2,
		...
		if assert.Equal(t, http.StatusOK, res.StatusCode, "Invalid result status code '%s'", res.Status) {
			...
			if assert.NotEmpty(t, ee) {
				for _, e := range ee {
					...
					assert.NotEmpty(t, e.CustomerDisplayName)
					assert.NotEmpty(t, e.CustomerEthereumAddress)
				}
			}
		}
	})
...
func TestJobEdit(t *testing.T) {
	...
	t.Run("put•all fields", func(t *testing.T) {
		...
		if assert.Equal(t, http.StatusOK, res.StatusCode, "Invalid result status code '%s'", res.Status) {
			...
			if assert.NotEmpty(t, e) {
				...
				assert.True(t, decimal.RequireFromString("45.01").Equal(e.Budget))
				...
				if assert.NoError(t, err) {
					...
					assert.Equal(t, "45.01", d.Budget.String)
	...
	t.Run("put•budget is missing", func(t *testing.T) {
		theJob, err := queries.JobAdd(ctx, pgdao.JobAddParams{
			ID:          pgdao.NewID(),
			Title:       "Title before change",
			Description: "Description before change",
		...
		if assert.Equal(t, http.StatusOK, res.StatusCode, "Invalid result status code '%s'", res.Status) {
			...
			if assert.NotEmpty(t, e) {
				assert.NotEmpty(t, e.ID)
				assert.True(t, decimal.RequireFromString("0.00").Equal(e.Budget))
	...
	t.Run("put•budget is zero", func(t *testing.T) {
		theJob, err := queries.JobAdd(ctx, pgdao.JobAddParams{
			ID:          pgdao.NewID(),
			Title:       "Title before change",
			Description: "Description before change",
	...
	t.Run("put•duration is missing", func(t *testing.T) {
		theJob, err := queries.JobAdd(ctx, pgdao.JobAddParams{
			ID:          pgdao.NewID(),
			Title:       "Title before change",
			Description: "Description before change",
		...
		if assert.Equal(t, http.StatusOK, res.StatusCode, "Invalid result status code '%s'", res.Status) {
			...
			if assert.NotEmpty(t, e) {
				assert.NotEmpty(t, e.ID)
				assert.Equal(t, int32(0), e.Duration)

				d, err := queries.JobGet(ctx, theJob.ID)
				if assert.NoError(t, err) {
					assert.Equal(t, theJob.ID, d.ID)
					assert.Equal(t, int32(0), d.Duration.Int32)
				}
			}
		}
	})

	t.Run("put•duration is negative", func(t *testing.T) {
		theJob, err := queries.JobAdd(ctx, pgdao.JobAddParams{
			ID:          pgdao.NewID(),
			Title:       "Title before change",
			Description: "Description before change",
